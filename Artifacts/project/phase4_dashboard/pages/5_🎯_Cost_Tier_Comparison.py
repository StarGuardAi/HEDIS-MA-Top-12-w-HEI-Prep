...
render_standard_sidebar(
    membership_slider_key="membership_slider_cost_tier_comparison",
    start_date_key="sidebar_start_date_cost_tier_comparison",
    end_date_key="sidebar_end_date_cost_tier_comparison"
)
...
<div class="page-title-container">
    <h1>🎯 Cost Tier Comparison</h1>
</div>
""", unsafe_allow_html=True)
st.markdown("<p style='text-align: center; margin-top: 0; margin-bottom: 0.75rem; font-size: 1rem;'>Analyze performance across Low Touch, Medium Touch, and High Touch intervention tiers</p>", unsafe_allow_html=True)

# Display date range info
col1, col2 = st.columns([1, 1], gap="small")
with col1:
    st.info("💰 Cost tiers help optimize intervention mix for maximum ROI")
with col2:
...
try:
    ...
    if df.empty:
        ...
        if 'interventions_count' in df_scaled.columns:
        ...
        # Summary metrics
        if 'success_rate' in df_scaled.columns:
            avg_success = df_scaled['success_rate'].mean()
            total_interventions = df_scaled['interventions_count'].sum() if 'interventions_count' in df_scaled.columns else 0
            total_closures = df_scaled['successful_closures'].sum() if 'successful_closures' in df_scaled.columns else 0
            total_investment = df_scaled['total_investment'].sum() if 'total_investment' in df_scaled.columns else 0
            
            st.header("📊 Cost Tier Summary")
            ...
            with col3:
                st.metric("Successful Closures", f"{int(total_closures):,}")
            with col4:
        ...
        # Create chart
        if 'cost_tier' in df_scaled.columns and 'avg_cost' in df_scaled.columns:
            y_cols = ["avg_cost"]
            if 'cost_per_closure' in df_scaled.columns:
                y_cols.append("cost_per_closure")
            ...
            st.plotly_chart(fig, use_container_width=True, config={'responsive': True, 'displayModeBar': False}, key="cost_tier_main_chart")
        ...
        with col1:
            if 'success_rate' in df_scaled.columns and 'cost_tier' in df_scaled.columns:
                ...
                st.plotly_chart(fig_success, use_container_width=True, config={'responsive': True, 'displayModeBar': False}, key="cost_tier_success_bar")
        ...
        with col2:
            if 'total_investment' in df_scaled.columns and 'cost_tier' in df_scaled.columns:
                ...
                st.plotly_chart(fig_investment, use_container_width=True, config={'responsive': True, 'displayModeBar': False}, key="cost_tier_investment_bar")
        ...
        if 'cost_per_closure' in df_scaled.columns and 'success_rate' in df_scaled.columns:
            ...
            st.plotly_chart(fig_efficiency, use_container_width=True, config={'responsive': True, 'displayModeBar': False}, key="cost_tier_efficiency_scatter")
        
        st.divider()
        
        # Radar Chart: Cost Tier Multi-Dimensional Comparison
        if len(df_scaled) > 0 and 'cost_tier' in df_scaled.columns:
            st.header("🎯 Cost Tier Multi-Dimensional Comparison")
            st.markdown("**Radar Chart:** Compare cost tiers across multiple performance dimensions")
            
            # Aggregate by cost tier
            tier_agg = df_scaled.groupby('cost_tier').agg({
                'success_rate': 'mean',
                'cost_per_closure': 'mean',
                'total_investment': 'sum',
                'successful_closures': 'sum'
            }).reset_index()
            
            # Normalize values for radar chart
            tier_agg['success_normalized'] = tier_agg['success_rate'].round(1)
            tier_agg['cost_efficiency'] = (100 - (tier_agg['cost_per_closure'] / tier_agg['cost_per_closure'].max() * 100)).round(1)
            tier_agg['investment_normalized'] = (tier_agg['total_investment'] / tier_agg['total_investment'].max() * 100).round(1)
            tier_agg['closures_normalized'] = (tier_agg['successful_closures'] / tier_agg['successful_closures'].max() * 100).round(1)
            ...
            for idx, row in tier_agg.iterrows():
                radar_data.append({
                    ...
                    'Cost Efficiency': row['cost_efficiency'],
                    'Total Investment': row['investment_normalized'],
                    'Successful Closures': row['closures_normalized']
                })
            ...
            fig_radar = create_wow_radar_chart(
                df=radar_chart_df,
                categories=['Success Rate', 'Cost Efficiency', 'Total Investment', 'Successful Closures'],
                group_col='cost_tier',
                title="🎯 Cost Tiers: Multi-Dimensional Performance",
            ...
            with col1:
                st.plotly_chart(fig_radar, use_container_width=True, config={'responsive': True, 'displayModeBar': False}, key="cost_tier_radar_chart")
            with col2:
                st.markdown("**Tier Legend:**")
                st.dataframe(tier_agg[['cost_tier', 'success_rate', 'cost_per_closure']], use_container_width=True, hide_index=True)
                st.caption("💡 Compare tiers across all performance dimensions")
        ...
        with st.expander("📋 View Detailed Data"):
            ...
            if 'measure_name' in display_df.columns:
                display_df = display_df[['cost_tier', 'interventions_count', 'successful_closures', 'avg_cost', 'success_rate', 'total_investment', 'cost_per_closure']]
            else:
                display_df = display_df[['cost_tier', 'interventions_count', 'successful_closures', 'avg_cost', 'success_rate', 'total_investment', 'cost_per_closure']]
