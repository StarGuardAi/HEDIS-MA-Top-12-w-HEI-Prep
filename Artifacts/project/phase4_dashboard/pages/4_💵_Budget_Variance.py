...
render_standard_sidebar(
    membership_slider_key="membership_slider_budget_variance",
    start_date_key="sidebar_start_date_budget_variance",
    end_date_key="sidebar_end_date_budget_variance"
)
...
<div class="page-title-container">
    <h1>💵 Budget Variance by Measure</h1>
</div>
""", unsafe_allow_html=True)
st.markdown("<p style='text-align: center; margin-top: 0; margin-bottom: 0.75rem; font-size: 1rem;'>Compare budget allocations vs actual spending across HEDIS measures</p>", unsafe_allow_html=True)

# Display date range info
col1, col2 = st.columns([1, 1], gap="small")
with col1:
    st.info("💰 Budget tracking helps identify cost overruns and optimization opportunities")
with col2:
...
try:
    ...
    if df.empty:
        ...
        if 'variance' in df.columns:
            ...
            under_budget_count = len(df[df['variance'] < 0])
            on_budget_count = len(df[df['variance'] == 0])
            
            st.header("📊 Budget Summary")
            col1, col2, col3, col4 = st.columns(4)
            with col1:
                st.metric("Total Variance", f"${total_variance:,.2f}", delta=None)
            with col2:
                st.metric("Over Budget", f"{over_budget_count}", delta=None)
            with col3:
                st.metric("Under Budget", f"{under_budget_count}", delta=None)
            with col4:
                st.metric("On Budget", f"{on_budget_count}", delta=None)
        ...
        if 'measure_code' in df.columns and 'budget_allocated' in df.columns and 'actual_spent' in df.columns:
            ...
            st.plotly_chart(fig, use_container_width=True, config={'responsive': True, 'displayModeBar': False}, key="budget_variance_main_chart")
        ...
        with col1:
            if 'variance_pct' in df.columns and 'measure_code' in df.columns:
                ...
                st.plotly_chart(fig_var_pct, use_container_width=True, config={'responsive': True, 'displayModeBar': False}, key="budget_variance_pct_bar")
        ...
        with col2:
            if 'budget_status' in df.columns:
                ...
                st.plotly_chart(fig_status, use_container_width=True, config={'responsive': True, 'displayModeBar': False}, key="budget_variance_status_pie")
        ...
        if 'budget_allocated' in df.columns and 'actual_spent' in df.columns:
            ...
            # Create temporary column with absolute variance for size (Plotly requires non-negative sizes)
            df_scatter = df.copy()
            if 'variance' in df_scatter.columns:
                df_scatter['variance_abs'] = df_scatter['variance'].abs()
            fig_scatter = create_scatter_plot(
                df_scatter,
                x_col="budget_allocated",
                y_col="actual_spent",
                size_col="variance_abs" if 'variance_abs' in df_scatter.columns else None,
                color_col="variance_pct" if 'variance_pct' in df_scatter.columns else None,
                title="Budget Allocated vs Actual Spent",
                x_label="Budget Allocated ($)",
                y_label="Actual Spent ($)"
            )
            st.plotly_chart(fig_scatter, use_container_width=True, config={'responsive': True, 'displayModeBar': False}, key="budget_variance_scatter")
        ...
        with st.expander("📋 View Detailed Data"):
            ...
            if 'measure_name' in display_df.columns:
                display_df = display_df[['measure_code', 'measure_name', 'budget_allocated', 'actual_spent', 'variance', 'variance_pct', 'budget_status']]
                display_df.columns = [
                    "Measure Code",
                    "Measure Name",
                    "Budget Allocated ($)",
